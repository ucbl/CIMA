...
public class Activator implements BundleActivator {
	...
	public void start(BundleContext bundleContext) throws Exception {
		logger.info("Register IpuService in nscl controller..");
		bundleContext.registerService(IpuService.class.getName(), new InfrastructureController(), null);
		ServiceReference refServ = bundleContext.getServiceReference(AvatarService.class.getName());
		if(refServ != null) {
			this.avatarService = (AvatarService)bundleContext.getService(refServ);
			logger.info("Avatar service is getting successfully");
		}
		...
		sclServiceTracker = new ServiceTracker<Object, Object>(bundleContext, SclService.class.getName(), null) {
			...
			public Object addingService(ServiceReference<Object> reference) {
				...
				final DeviceManager ipuManager = new DeviceManager(sclService, avatarService);
				new Thread(){
					public void run(){
						try {
							ipuManager.start();
