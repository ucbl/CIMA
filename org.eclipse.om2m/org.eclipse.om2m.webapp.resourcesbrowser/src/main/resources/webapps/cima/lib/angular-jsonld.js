  ...
  function JsonldRest($q, $log, $rootScope, Restangular, jsonld, jsonldContext) {
    ...
    function restangularize(node, parent){
        ...
        for (var field in node) {
          // Added by TI5 - CIMA
          // if (field.indexOf("#") > 0) {
          //     var k = field.substr(field.indexOf('#') + 1);
          //     node[k] = node[field];
          //     //delete node[field];
          // }
          // Added by TI5 - CIMA
          ...
          // if (field !== '@type' && field.indexOf(node['@type']) >= 0) {
          //   var k = field.substr(node['@type'].length + 1);
          //   var temp = node[field];
          //   node[k] = temp;
          //   delete node[field];
          // }
        ...
        if(node['@context']){
          ...
          for(var prop in context){
            if (context[prop] in node) {
              // Added by TI5 - CIMA
              var temp = node[context[prop]];
              node[prop] = temp;
              delete node[context[prop]];
              // Added by TI5 - CIMA
            }
